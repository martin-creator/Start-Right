...
impl AgentReactFrontendDeveloper {
    ...
    async fn assign_api_routes(
        ...
        let msg_context: String = format!(
            "WEBSITE SPECIFICATION: {{PROJECT_DESCRIPTION: {},PAGES: {:?},INTERNAL_API_ROUTES: {},EXTERNAL_API_ROUTES: {} }}",
            project_description,
...
pub mod tests {
    ...
    async fn develops_context_and_branding() {
        ...
        let mut agent: AgentReactFrontendDeveloper = AgentReactFrontendDeveloper::new();

        // Initialze Factsheet
        let mut factsheet: FactSheet = serde_json
            ::from_str(
                "{\"project_description\":\"Build a todo app for a fitness tracking goal\",\"project_scope\":{\"is_crud_required\":true,\"is_user_login_and_logout\":true,\"is_external_urls_required\":true},\"external_urls\":[\"https://api.exchangeratesapi.io/latest\"],\"backend_code\":null,\"frontend_code\":null,\"json_db_schema\":null}"
            )
            .unwrap();
